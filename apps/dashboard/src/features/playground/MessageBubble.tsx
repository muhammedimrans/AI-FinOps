...
interface MessageBubbleProps {
  ...
  onRegenerate: (turn: ConversationTurn) => void;
  onContinue: (turn: ConversationTurn) => void;
  onExport: (turn: ConversationTurn) => void;
  onDelete: (turn: ConversationTurn) => void;
  onCostAnalysisOpen: () => void;
}

/** One conversation exchange — a user bubble followed by the assistant's
 * response card, with the full response-actions row (redesign goal #4) and
 * the statistics/cost-analysis panels (goals #7/differentiator) attached to
 * every completed assistant turn. */
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2 justify-end">
        <div className="group relative max-w-[85%] rounded-2xl rounded-tr-sm bg-primary px-3.5 py-2.5 text-sm text-white whitespace-pre-wrap shadow-card">
          {turn.isContinuation && (
        ...
        <div className="flex flex-col items-center gap-1 pt-1 flex-shrink-0">
          ...
          <button
            type="button"
            onClick={() => onCopy(turn.userPrompt)}
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0 rounded-2xl rounded-tl-sm border border-border-subtle bg-app-card px-3.5 py-2.5">
          {turn.isPending ? (
            ...
            <div className="flex flex-col gap-2.5">
              <div className="text-sm text-tx-primary [&_pre]:my-0">
                {renderMarkdown(turn.execution.response_text ?? "")}
              </div>

              <div className="flex flex-wrap items-center gap-1 border-t border-border-subtle pt-2">
                <ActionButton icon={Copy} label="Copy" onClick={() => onCopy(turn.execution!.response_text ?? "")} />
                <ActionButton icon={RefreshCw} label="Regenerate" onClick={() => onRegenerate(turn)} />
                <ActionButton
                  icon={Sparkles}
                  label="Continue"
                ...
                />
                <ActionButton icon={Download} label="Export" onClick={() => onExport(turn)} />
