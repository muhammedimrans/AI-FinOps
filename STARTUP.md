...
#### Testing Google Gemini (EP-26.0.3.1)

A step-by-step checklist for confirming a Google connection actually works, separate from the walkthrough above (which explains *how* to connect; this explains *how to verify* the connection once it's made):

1. **Create an AI Studio API key.** [aistudio.google.com](https://aistudio.google.com) → **Get API key** → **Create API key**. No credit card required for the free tier. Copy the key (`AIza...`) immediately — AI Studio does not show it again.
2. **Connect it in Costorah.** Connections → Add connection → select **Google Gemini** from the provider dropdown → paste the key → Save. The provider dropdown shows the real Google logo next to the option, and the connection row (once saved) shows the same logo plus a **"AI Studio · Gemini API"** platform/service badge — confirming you connected the AI Studio surface, not something else.
3. **Verify it works — click "Test Connection."** As of this EP, the result panel shown directly under the connection (no need to check a separate tab or page) displays:
   - **Connected successfully** (or **Connection test failed**, with the specific reason) — driven by the same live `GET /v1beta/models` call used for validation.
   - **Provider: Google Gemini**, **Platform: AI Studio**, **Service: Gemini API** — sourced from Costorah's Provider Brand Registry (EP-26.0.4), not re-fetched, so this appears instantly.
   - **API status: Reachable** — confirms Costorah's server successfully reached Google's endpoint with your key.
   - A capability tag row (Chat, Vision, Tools, Streaming, etc.) — cosmetic, for quick recognition, not itself a live capability probe.
4. **Which Gemini models should appear?** Whatever your key currently has access to — Costorah pulls this live from `GET /v1beta/models` (EP-26.0.2), paginated, so it always reflects Google's real, current catalog rather than a list Costorah maintains by hand. As of this writing that's typically the Gemini 2.5 line (Pro / Flash / Flash-Lite) and newer; if Google ships a new model tomorrow, it appears in Costorah's Connect Provider form automatically, with no Costorah deployment required. If the live call ever fails, Costorah falls back to a small static list so the form is never empty — but the live catalog is always tried first.
5. **Trigger a manual sync ("Sync Now").** The sync will complete successfully (status: **Synced**), and **0 usage records will be imported** — this is the expected, correct outcome for every Google AI Studio connection, not a failure. The connection's sync-status panel shows this plainly rather than implying anything was missed.
6. **Why is usage history unavailable?** Google's AI Studio / Gemini Developer API — the surface an `AIza...` key authenticates against — has no bulk, key-scoped usage-history endpoint at all. There is nothing for Costorah to call; this is a gap on Google's own platform, not a Costorah limitation, and it is disclosed identically everywhere a Google connection appears (Connections page, Test Connection panel, sync status, this doc). See "Platform vs. Vertex AI" above for exactly where that data *does* live (Vertex AI's Cloud Billing Export) and why it requires a different, GCP-service-account-shaped credential Costorah doesn't connect to today.
7. **Confirm the scheduler picks it up.** Once `auto_sync_enabled` is on for your organization (Settings → Workspace), the background scheduler (EP-23.4) syncs every active connection, Google included, on the configured interval — identically to OpenAI/Anthropic. You'll see the connection's "Last sync" timestamp advance on schedule with 0 records imported each time.
8. **Confirm Dashboard/Analytics render correctly.** With 0 usage rows, Overview and Analytics show the connection as a healthy, present provider with $0 spend — not an error state, not a missing row, not a fabricated number. This is exactly what §17/§21's empty-state design already handles for any provider with legitimately zero usage.

**No live Google AI Studio credential was available in this sandbox to run this checklist against a real account** — every step above was validated via this codebase's existing hermetic test suites (`test_ep26_0_2_google.py`, `test_ep26_0_2_1_lifecycle.py`) against realistic mocked HTTP responses, and via direct source-code review of `GoogleProvider`/`Connections.tsx`, not a first-party API response this session observed. This is the same disclosed limitation every prior provider-validation EP in this document has carried (§32, §33, EP-26.0.2, EP-26.0.2.1) — a future session with a real key should walk through the 8 steps above against the live product and correct anything that doesn't match.
